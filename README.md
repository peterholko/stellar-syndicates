...
| **Sensor bubbles: soft gradient edge (declutter, OPTION B)** *(branch `async-automation`)* | ✅ **Complete** | The sensor-coverage union stacks many circles (command center + every own ship + the scout's oversized bubble + owned Sensor-Array systems), and drawn as hard teal rings they walled off the map. Replaced the boundary stroke entirely with a **soft radial VIGNETTE** — each bubble is a gentle "sensed region" glow that fades to full transparency at its rim, so overlapping coverage blends pleasantly instead of stacking crisp circles. **ONE tunable style block** (`SENSOR_COVERAGE = { layers: 8, peakAlpha: 0.09 }`): a true radial gradient is awkward per-frame, so it's approximated by `layers` concentric filled discs of equal low alpha (`peakAlpha/layers` ≈ 0.011 each) — alpha accumulates to ≈peakAlpha at the center and falls ~linearly to ≈0 at the rim (the rim is one disc ≈0.011, no visible edge). Cheap: just triangles in the `sensorGfx` Graphics that's already cleared each frame — **no per-frame gradient/texture rebuild**. Applied uniformly to all coverage sources; owner-only + own-vs-rival gating **unchanged** (purely how the owner's own coverage is drawn). *(Supersedes the brief dashed-ring OPTION A.)* **Deliberately left distinct/crisp:** the dashed cyan **platform protection ring** (same function, untouched), the pulsing red **threat/blockade** rings (`COL_THREAT`), the **battle-marker** attention pulses, and the white **selection** ring. Client/UX only. tsc + vite build clean; verified live (soft edge-less glow at fit + deep zoom, no hard border, other rings unaffected, no console errors). |
