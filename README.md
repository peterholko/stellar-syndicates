...
**Formation rule (§14.2 — the slowest member sets the pace).** A fleet's
acceleration is `min over present kinds (thrust ÷ hull_mass) × hull ÷ (hull +
cargo)` and its cruise cap is `min over present kinds (max_speed)`. Total mass is
`Σ hull_mass(kind) × count + cargo`, so **fuel ∝ distance × total mass** exactly
as before. Consequences: a raider "hammer" carrying a colony ship *lumbers* at
the colony's pace, telegraphing itself by physics; a loaded convoy fleet-of-one
reduces to the old `thrust/(hull+cargo)` — byte-identical.
...
**Combat (Part 1).** Still resolves as it did — each fleet fights as its
**flagship** kind (precedence colony > convoy > corvette > raider > scout), so a
fleet-of-one is identical to the old ship. *Part 2 replaces this with
deterministic per-tick Lanchester attrition (proportional casualties).*
