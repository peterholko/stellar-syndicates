...
| **Sensor bubbles: dashed + faint (declutter, OPTION A)** *(branch `async-automation`)* | ✅ **Complete** | The sensor-coverage union stacks many circles (command center + every own ship + the scout's oversized bubble + owned Sensor-Array systems), and drawn as SOLID teal rings + a fill they walled off the map. Now each bubble draws as a **DASHED outer boundary** (via the existing screen-px `dashedCircle`, so the pattern reads consistently at any zoom) over a **whisper of interior tint** — a soft "coverage hint," not a hard border. **ONE tunable style block** (`SENSOR_COVERAGE = { dashOn: 8, dashOff: 6, outlineAlpha: 0.28, fillAlpha: 0.05 }`) drives all sources uniformly (fill 0.045→0.05, solid stroke 0.14→dashed 0.28; set `fillAlpha: 0` to drop the tint entirely). Owner-only + own-vs-rival gating **unchanged** — purely how the owner's own coverage is drawn. **Deliberately left distinct:** the dashed cyan **platform protection ring** (same function, untouched), the pulsing red **threat/blockade** rings (`COL_THREAT`), the **battle-marker** attention pulses, and the white **selection** ring all keep their solid/pulsing treatments. Client/UX only. tsc + vite build clean; verified live (dashed faint bubbles at fit + deep zoom, dashes scale cleanly, other rings unaffected, no console errors). |
