...
ws.addEventListener("open", () => ws.send(encodeMessage({ type: "Join", name: "Raider Co" })));
...
const main = async () => {
  ...
  while (Date.now() < deadline && reports.length === 0) {
    ...
    if (raider && convoyId) ws.send(encodeMessage({ type: "CommitRaid", raider_id: raider.id, target_id: convoyId }));
    await sleep(1200);
  }
  const rep = reports[0];
