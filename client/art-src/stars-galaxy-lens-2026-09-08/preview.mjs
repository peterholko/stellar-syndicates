...
const previous = new URL('../stars-2026-09-08/', root);
const manifest = JSON.parse(await readFile(new URL('prompts.json', root), 'utf8'));
const old = JSON.parse(await readFile(new URL('prompts.json', previous), 'utf8'));
...
await text(overview, 'STELLAR SYNDICATES · GALAXY-MAP LENS STARS', 0, 22, 1600, 27);
...
await text(comparison, 'DETAILED SYSTEM ART / GALAXY LENS ART', 0, 24, 1600, 27);
await text(comparison, 'Equal visible extents for artwork comparison — both sets preserved; live game unchanged', 0, 67, 1600, 16, '#8b9db3');
for (const [i, star] of manifest.stars.entries()) {
  ...
  for (const [j, path] of [oldFile, file].entries()) {
    ...
    await text(comparison, j ? 'GALAXY' : 'DETAILED', x + 24 + j * 144, cy + 197, 128, 13, '#8b9db3');
  }
}
