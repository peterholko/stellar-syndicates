...
export class DeckRosterRoutes {
  ...
  private researchHtml(): string {
    ...
    const queueHtml = queue.length ? queue.map((id, index) => {
      ...
      return `<div class="deck-research-queue-row"><span><b>${index + 1}</b><span><strong>${esc(programme?.name ?? id)}</strong><small>${index === 0 ? "active/front" : "queued"}</small></span></span><div><button type="button" data-deck-act="research-up" data-index="${index}" ${index === 0 ? "disabled" : ""}>↑</button><button type="button" data-deck-act="research-down" data-index="${index}" ${index === queue.length - 1 ? "disabled" : ""}>↓</button><button type="button" data-deck-act="research-remove" data-index="${index}">Remove</button></div></div>`;
