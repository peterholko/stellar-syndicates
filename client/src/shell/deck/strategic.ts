...
export class DeckStrategicRoutes {
  ...
  render(route: DeckRoute | null, force = false): boolean {
    ...
    if (!force && active instanceof HTMLInputElement && this.root.contains(active)) return true;
    ...
    if (route.name === "operations") setHtml(this.root, this.operationsHtml());
  ...
  handleAction(button: HTMLButtonElement, route: DeckRoute | null): boolean {
    ...
    if (action.startsWith("strategic-operation-")) this.operationAction(action, button);
    ...
    else if (action === "strategic-battle-withdraw" && button.dataset.fleet) {
      this.ctx.send({ type: "Withdraw", fleet_id: button.dataset.fleet });
      this.hooks.notice("<b>Withdraw order sent</b> · light-delayed to the engaged fleet.");
      ...
      if (Number.isFinite(id)) {
        this.ctx.state.battleDismissed.add(id);
  ...
  handleInput(input: HTMLInputElement, route: DeckRoute | null): boolean {
  ...
  private operationsHtml(): string {
    const [stage, stageCopy] = MIDGAME_COPY[this.ctx.state.midgameStage];
    const available = this.ctx.state.operations.filter((o) => o.state === "offered" || (o.state === "active" && !o.joined));
    const active = this.ctx.state.operations.filter((o) => o.state === "active" && o.joined);
    const history = this.ctx.state.operations.filter((o) => !["offered", "active"].includes(o.state)).sort((a, b) => b.reported_at - a.reported_at).slice(0, 12);
    const group = (title: string, rows: OperationView[], empty = "") => `<section class="deck-section"><header><div><h3>${esc(title)}</h3><p>${rows.length ? `${rows.length} arrived record${rows.length === 1 ? "" : "s"}.` : empty}</p></div><b>${rows.length}</b></header>${rows.map((o) => this.operationCard(o)).join("") || `<div class="deck-empty-inline">${esc(empty)}</div>`}</section>`;
    return `<section class="deck-page deck-operations"><header class="deck-page__lead"><span>Contracts · objectives · shared projects</span><h2>${esc(stage)}</h2><p>${esc(stageCopy)}</p></header>${group("Active", active, "No operation is currently assigned.")}${group("Available", available, "No arrived offers. The board updates when fresh reports reach command.")}${history.length ? group("History", history) : ""}</section>`;
  }
  ...
  private operationCard(operation: OperationView): string {
    ...
    return `<article class="deck-operation is-${operation.state}">${hullArt ? `<img src="${escAttr(hullArt)}" alt="">` : ""}<div><header><span>${operationIcon(operation)}<b>${esc(operationTitle(operation))}</b></span><em>${esc(human(operation.state))}</em></header><p>${esc(operationCopy(operation))}</p><div class="deck-meter"><i style="width:${pct.toFixed(1)}%"></i></div><footer><span>${operation.progress}/${operation.goal}</span><span>${esc(operationReward(operation))}</span><span>${esc(time)}</span><span>Report ${fmtDur(Math.max(0, liveSimTime() - operation.reported_at))} old</span></footer>${actions.length ? `<div class="deck-operation__actions">${actions.join("")}</div>` : ""}</div></article>`;
  }
  ...
  private operationAction(action: string, button: HTMLButtonElement): void {
    ...
    else if (action === "strategic-operation-assign" && this.ctx.state.selectedShipId) this.ctx.send({ type: "AssignOperationFleet", operation_id, fleet_id: this.ctx.state.selectedShipId });
    else if (action === "strategic-operation-recover" && this.ctx.state.selectedShipId) this.ctx.send({ type: "RecoverOperation", operation_id, fleet_id: this.ctx.state.selectedShipId });
  ...
  private ongoingBattleHtml(battle: import("../../protocol").BattleView): string {
    ...
    const withdraw = own.map((fleet) => {
      const pending = latestPendingOrder(fleet.id);
      const timing = pending ? liveSimTime() < pending.arrives_at ? ` · signal ${fmtDur(Math.max(0, pending.arrives_at - liveSimTime()))}` : ` · response ${fmtDur(Math.max(0, pending.response_at - liveSimTime()))}` : "";
      return `<button type="button" data-deck-act="strategic-battle-withdraw" data-fleet="${escAttr(fleet.id)}">Withdraw ${esc(fleetName(fleet))}${esc(timing)}</button>`;
    }).join("");
    return `<section class="deck-page deck-battle-route"><header class="deck-page__lead"><span>Battle raging · as of ${fmtDur(battle.age)} ago</span><h2>Engagement ${esc(nearestSystemName(battle.pos))}</h2><p>Raging ${fmtDur(elapsed)} · forces remaining by your arrived light.</p></header><section class="deck-section deck-force-strip"><header><div><h3>Observed forces</h3><p>${battle.own ? "Exact own composition; rival strength remains bucketed." : "Weapons-fire light reveals only the observed rivals."}</p></div></header>${battle.own ? forceSide("You", ownChips) : ""}${forceSide(battle.own ? "Enemy" : "Forces", rivalChips)}</section>${delay !== null ? `<div class="deck-alert${delay > 20 ? " deck-alert--bad" : ""}"><b>Order lag · ${fmtDur(delay)}</b><span>A command issued now arrives around ${arrivalLocal(delay)}${delay > 20 ? " · too far to steer closely" : " · still in reach"}.</span></div>` : ""}<div class="deck-battle-actions">${withdraw}${record ? `<button type="button" class="is-primary" data-deck-act="strategic-battle-view" data-record="${escAttr(record.id)}">Follow battle · delayed</button>` : ""}${battle.own ? `<button type="button" data-deck-act="strategic-battle-doctrine">Fleet doctrine</button>` : ""}</div></section>`;
  }

  private battleReportHtml(report: import("../../protocol").BattleReportView): string {
    this.markViewed(report.id);
    ...
    const verdict = ownDestroyed && rivalDestroyed ? "Mutual destruction" : ownDestroyed ? "Defeat · your force destroyed" : rivalDestroyed ? "Victory · rival force destroyed" : "Withdrawal · both sides survive";
    const record = recordForReport(report);
    return `<section class="deck-page deck-battle-route"><header class="deck-page__lead"><span>Battle result · delayed report</span><h2>Engagement ${esc(nearestSystemName(report.pos))}</h2><p>${esc(verdict)}</p></header><div class="deck-stat-grid"><dl class="deck-stat"><dt>Concluded</dt><dd>${fmtDur(Math.max(0, liveSimTime() - report.at_time))} ago</dd></dl><dl class="deck-stat"><dt>Learned</dt><dd>${fmtDur(Math.max(0, liveSimTime() - report.learned_at))} ago</dd></dl><dl class="deck-stat"><dt>Information delay</dt><dd>${fmtDur(Math.max(0, report.learned_at - report.at_time))}</dd></dl><dl class="deck-stat"><dt>Your role</dt><dd>${esc(human(report.you))}</dd></dl></div><section class="deck-section"><header><div><h3>Losses</h3><p>Outcomes are those carried by the report; the site may look different now.</p></div></header><div class="deck-loss-row"><span>You lost</span><b>${esc(losses(ownLost))}</b></div><div class="deck-loss-row"><span>They lost</span><b>${esc(losses(rivalLost))}</b></div></section><div class="deck-battle-actions">${record ? `<button type="button" class="is-primary" data-deck-act="strategic-battle-view" data-record="${escAttr(record.id)}">View battle replay</button>` : ""}<button type="button" data-deck-act="strategic-battle-dismiss" data-report="${report.id}">Dismiss map marker</button></div><p class="deck-muted">Dismissing removes only the map marker; this report remains in the Log.</p></section>`;
  }
  ...
  private captureReportHtml(report: import("../../protocol").CaptureReportView): string {
    ...
    return `<section class="deck-page deck-battle-route"><header class="deck-page__lead"><span>Capture · delayed report</span><h2>${report.captor ? "Captured" : "Lost"} ${esc(nearestSystemName(report.pos))}</h2><p>${report.captor ? "Your marines took the ground; the old owner keeps surviving fleets." : "Only the territory changed hands; surviving fleets remain in play."}</p></header><div class="deck-stat-grid"><dl class="deck-stat"><dt>System fell</dt><dd>${fmtDur(Math.max(0, liveSimTime() - report.at_time))} ago</dd></dl><dl class="deck-stat"><dt>Learned</dt><dd>${fmtDur(Math.max(0, liveSimTime() - report.learned_at))} ago</dd></dl><dl class="deck-stat"><dt>Information delay</dt><dd>${fmtDur(Math.max(0, report.learned_at - report.at_time))}</dd></dl></div><section class="deck-section"><header><div><h3>${report.captor ? "Plunder seized" : "Plunder lost"}</h3><p>The besieged stockpile changed hands; development tiers transfer at half strength.</p></div></header><div class="deck-alert"><b>${esc(plunder)}</b><span>One Troop Transport was consumed by the landing.</span></div></section><button type="button" data-deck-act="strategic-battle-dismiss" data-report="${report.id}">Dismiss map marker</button><p class="deck-muted">Dismissing removes only the map marker; this report remains in the Log.</p></section>`;
  }

  private markViewed(id: number): void {
    if (this.ctx.state.battleViewed.has(id)) return;
    this.ctx.state.battleViewed.add(id);
    saveBattleMarks();
  }
...
function fleetName(fleet: GhostView): string {
  return `${shipKindLabel(fleet.kind)} fleet`;
}
