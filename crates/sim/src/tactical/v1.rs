...
use super::rules_v1::{Rng, hull_affinity, hull_mass, max_speed, attack_weight, offense, featured};
...
/// External roster changes only. Ordinary damage/deaths are reproduced by
/// stepping the engine and therefore do not need a per-shot input log.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct RosterDelta {
    pub side: u8,
    pub departed: Vec<(EntityId, u32)>,
    pub arrived: Vec<(EntityId, Ship)>,
}
...
impl TacticalState {
    ...
    fn deploy_side(&mut self, side: u8, ships: &[(EntityId, Ship)]) {
        ...
        order.sort_by(|a, b| {
            hull_mass(b.1.kind)
                .partial_cmp(&hull_mass(a.1.kind))
    ...
    fn spawn(&mut self, side: u8, fleet: EntityId, ship: &Ship) {
        ...
        let hp = ship.hp.clamp(1.0, hull_mass(kind));
    ...
    pub fn sync(&mut self, desired: [&[(EntityId, Ship)]; 2]) -> [u32; 2] {
        self.sync_recorded(desired).0
    }

    pub(crate) fn sync_recorded(&mut self, desired: [&[(EntityId, Ship)]; 2]) -> ([u32; 2], Vec<RosterDelta>) {
        ...
        for side in 0..2u8 {
            ...
            let departed = self.combatants.iter()
                .filter(|c| c.side == side && !c.platform)
                .filter_map(|c| c.origin)
                .chain(self.waves[side as usize].iter().map(|w| (w.fleet, w.ship.id)))
                .filter(|id| !want.contains(id)).collect::<Vec<_>>();
            ...
            if !departed.is_empty() || !joining.is_empty() {
                changes.push(RosterDelta { side, departed, arrived: joining });
            }
        }
        (scouts, changes)
    }

    pub(crate) fn apply_roster_delta(&mut self, changes: &[RosterDelta]) {
        for change in changes {
            self.combatants.retain(|c| c.side != change.side || c.platform
                || c.origin.is_none_or(|id| !change.departed.contains(&id)));
            self.waves[change.side as usize].retain(|w| !change.departed.contains(&(w.fleet, w.ship.id)));
            for (fleet, ship) in &change.arrived {
                self.spawn(change.side, *fleet, ship);
            }
        }
    }
        ...
        for side in 0..2u8 {
            loop {
                ...
                let Some(best) = self.waves[side as usize]
                    ...
                    .enumerate()
                    .max_by(|(ai, a), (bi, b)| {
                        hull_mass(a.ship.kind)
                            .partial_cmp(&hull_mass(b.ship.kind))
                            .unwrap_or(std::cmp::Ordering::Equal)
    ...
    pub fn step(&mut self, raid: bool, mods: [SideMods; 2]) -> StepOutcome {
        ...
        for (i, want) in desired.iter().enumerate() {
            ...
            let max_speed = max_speed(c.kind) * ARENA_SCALE;
    ...
    pub(super) fn torpedo_threat_centroid(&self, enemy_side: u8) -> Option<Vec2> {
        let pts: Vec<Vec2> = self
            ...
            .filter(|c| {
                c.side == enemy_side
                    && c.hp > 0.0
                    && offense(&Loadout::from_key(&c.stack)).0 == DamageType::Torpedo
            })
            .map(|c| c.pos)
            .collect();
        if pts.is_empty() {
